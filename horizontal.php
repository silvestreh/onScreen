...
<header>
  <h1>onScreen: Load New Items</h1>
  <p>This demo shows you how to load new post horizontally.</p>
</header>
<section class="posts">
  <article class="post">
    ...
    <ul class="comments">
      <li>
        ...
        <div class="content">
          ...
          <p>Look, just because I don't be givin' no man a foot massage don't make it right for Marsellus to throw Antwone into a glass motherfuckin' house, fuckin' up the way the nigger talks. </p>
        </div>
      </li>
...
<script type="text/javascript">
  ...
  function appendPosts() {
    ...
    */
    $('footer').onScreen({
      ...
      doIn: function() {
        var $template = $('article.post').first().clone();
        
        $template.css({
          left: 40,                 // We're cloning a visible element and
          opacity: 0                // we need to reset the CSS we animated
        }).removeClass('onScreen'); // Also, remove the onScreen class
  			
  			// This is the part where you should do your ajax call and get
        // more stuff to append.
        $('div.loading').fadeIn('fast').delay(2000).fadeOut('fast',function() {
        	postWidth = $('article.post').outerWidth();
        	contentWidth = postWidth * $('article.post').length;
          $('section.posts').css({
  					'width':  contentWidth + postWidth + 580
  				});
          $(this).before($template);
          animatePosts(); // Add animations to appended elements
        });
      }
    });
  }
  ...
  $(function() {
    ...
    $('section.posts').css({
      width: $('article.post').outerWidth() * $('article.post').length + 170
    });
    
    $('header').width($(window).width()-122);
